...
mkdir -p \
  "${ROOT_DIR}/runtime/desktop-sessions" \
...
printf 'Next steps:\n'
printf '  1. Edit %s/.env\n' "${ROOT_DIR}"
printf '  2. Run make macos-capture-up when you want This Mac capture targets in the background\n'
printf '  3. Run make up-openclaw when you want the helper gateway (token comes from ~/.openclaw/openclaw.json)\n'
printf '  4. Run make macos-app-dev to start the native app\n'
printf '  5. Create isolated desktops from the app only when you need them\n'
